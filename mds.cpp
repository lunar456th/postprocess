...
{
	smat::Matrix<double> *X0 = new smat::Matrix<double>(argv[1]); // read the initialization matrix from txt file
	smat::Matrix<double> *D = new smat::Matrix<double>(X0->columns(), X0->columns());
	...
	{
		smat::Matrix<double> *v1 = X0->sub(0, X0->rows() - 1, i, i);
		for (int j = 0; j<X0->columns(); j++)
		{
			smat::Matrix<double> *v2 = X0->sub(0, X0->rows() - 1, j, j);
			v2->multiplyNumberSelf(-1);
			smat::Matrix<double> *diff = v1->addMatrixNew(v2);
			D->set(i, j, diff->pnorm(2));
	...
	D->print();
	D->saveTxt("distanceMatrix.txt");

	printf("\nBegin to run MDS(UCF): \n");

	smat::Matrix<double> * X2 = MDS_UCF(D, NULL, dim, iter); // without initialization
	printf("MDS done! \n");
	X2->saveTxt("MDS.txt");
